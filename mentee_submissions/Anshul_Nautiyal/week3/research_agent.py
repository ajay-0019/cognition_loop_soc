...
def search_the_web(query: str) -> str:
    print(f" -> Executing search_the_web with query: '{query}'...")
        ...
        try:
            ...
            for i in range(count):
                ...
                if link_el.count() > 0:
                    ...
                    if title and href:
                        valid_count += 1
                        formatted_results.append(f"Result {valid_count}:\nTitle: {title}\nURL: {href}\nSummary: {snippet}\n")
                        if valid_count >= 5:
                            break
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        question = input("Ask me a question that requires live data: ")
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are an expert research agent. You can search the live web to find current information, "
                "top stories, or real-time facts. Always prefer using search_the_web when the user asks about "
                "recent events, current status, or live data. You MUST use tools when you do not know the answer. "
                "Analyze the search results, and repeat searching if needed, until you can provide a complete, "
                "factual answer."
            )
        },
    ...
    round_num = 1
    while True:
        print(f"\n[Round {round_num}] Sending messages history to Groq...")
        try:
        ...
        for call in tool_calls_to_execute:
            ...
            print(f"\n[Tool Call] Agent requested tool: '{tool_name}'")
            ...
            if tool_name in available_tools:
                ...
                try:
                    ...
                    result = f"Error executing tool locally: {str(err)}"
            else:
                result = f"Error: Tool '{tool_name}' is not supported."
                
            print(f"[Tool Response] Received data back.")
